...
> **warning** Push notifications are not supported on Android Emulator or iOS Simulator. However, you can still use local notifications on these platforms.
...
- Obtain an Expo push token, so you can send push notifications with [Expo Push API](/push-notifications/sending-notifications/)
...
Firebase Cloud Messaging credentials are required for all Android apps to receive push notifications. For more information, see how to get [FCM credentials](/push-notifications/push-notifications-setup/#android) for your app.
...
- Starting from Android 12 (API level 31), to schedule the notification that triggers at an exact time, you need to add
...
| ---------------------------------- | ---------- | ----------------------------------------------------------------------- |
| false                              | Foreground | `NotificationReceivedListener` & `registerTaskAsync`                    |
| false                              | Background | `registerTaskAsync`                                                     |
| false                              | Killed     | `registerTaskAsync`                                                     |
...
See the `ExpoPushMessage` type in the [`expo-server-sdk`](https://github.com/expo/expo-server-sdk-node/blob/b470314023d8393073ecdbc07927b32d163af790/src/ExpoClient.ts#L346) package.
